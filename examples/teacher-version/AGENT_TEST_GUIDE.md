...
Branch: */main
Script Path: examples/teacher-version/Jenkinsfile-simple
```
...
# 提交代码到 Git
git add examples/teacher-version/
...
stage('Copy Project') {
    steps {
        sh """
            cp -r /mnt/d/Repositories/JenkinsDeploy/examples/teacher-version/todoapp-backend-api-main ${WORKSPACE}/
        """
    }
}
```
...
```bash
cd /mnt/d/Repositories/JenkinsDeploy/examples/teacher-version/todoapp-backend-api-main
dotnet test
```
...
- [.NET Agent 测试指南](../../DOTNET_AGENT_TEST_GUIDE.md)
- [Docker Agent 快速指南](../../DOCKER_AGENT_QUICKSTART.md)
- [Agent 部署对比](../../AGENT_DEPLOYMENT_COMPARISON.md)
